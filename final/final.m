...
% Question 2
% A = sym([1/sqrt(10) -3/sqrt(10); 3/sqrt(10) 1/sqrt(10)])
% A'

% Question 3

% A = [0.33 -0.75 0.44; 0.28 -0.25 -0.89; -0.90 -0.61 -0.15];
% B = [5.62 0 0; 0 0 0; 0 0 0];
% C = [0.22 -0.49 -0.84; 0.76 0.64 -0.12; 0.60 -0.62 0.51];
% D = A*B*C
% rref(D)
...
function e = fetchEigen(A)
	...
	I = x * I;
	B = A - I; % our lambda matrix
	e = eig(A); % returns vector of eigen values

	% [row, col]
	[i, j] = size(e);
	z = zeros(i, 1);
	% insert eigenvalue into B to produce a matrix to rref it and find eigenvalue
	for index = 1:i
		val = e(index, 1);
		eV = subs(B, x, val);
		vec = rref([eV z]); % remember to solve for leading terms to get eigenvector
		disp(["eigen value: " val " eigenvector: "])
		vec
	end
end
